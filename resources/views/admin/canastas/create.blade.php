...
<div class="container mx-auto mt-12 max-w-3xl">
    <h1 class="text-3xl font-bold mb-6">Crear Nueva Canasta</h1>
    ...
    <form action="{{ route('admin.canastas.store') }}" method="POST">
        ...
        @foreach($productos as $producto)
            <div class="mb-2 flex items-center">
                <input type="checkbox" name="productos[{{ $producto->id }}][id]" value="{{ $producto->id }}" class="mr-2">
                <label>{{ $producto->nombre }} (Disponible: {{ $producto->cantidad_disponible }})</label>
                <input type="number" name="productos[{{ $producto->id }}][cantidad]" min="1" value="1" class="border p-1 ml-4 w-20">
            </div>
        @endforeach

        <button type="submit" class="bg-green-500 text-white px-4 py-2 rounded-lg mt-6">Crear Canasta</button>
    </form>
</div>
@endsection